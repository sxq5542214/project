...
public class DictionaryController extends BaseController {
	...
	@RequestMapping("/dictionary/ajaxQueryAllDictionaryByCache.do")
	public ModelAndView ajaxQueryAllDictionaryByCache(HttpServletRequest request,HttpServletResponse response) throws IOException{
//		Map<String, Object> map = new HashMap<String, Object>();
		IOTWebDataBean result = new IOTWebDataBean();
		
		Map<String, Map<String, Map<String, String>>> cache = dictionaryService.getDictionarycache();
...
//			map.put("pd", pd);
	 ...
	 * 字典值管理,查询功能
	 */
	@RequestMapping("/admin/dictionary/ajaxQueryDictionaryList.do")
	public ModelAndView ajaxQueryDictionaryList(HttpServletRequest request,HttpServletResponse response) throws IOException{
		...
		try{
			...
			result =  dictionaryService.queryDictionaryList(bean);
		}catch (Exception e) {
			e.printStackTrace();
			log.error(e,e);
	...
	public ModelAndView deleteDictionary(HttpServletRequest request,HttpServletResponse response) throws IOException{
		try {
			...
			if(!StringUtil.isNull(bean.getId())){
				...
				bean.setCommons(configCruxService.getValueByTypeAndKey(ConfigCruxBean.CONFIG_CRUX_TYPE_POP_NEWS,ConfigCruxBean.CONFIG_CRUX_KEY_OPERATION_SUCCESS)) ;
			}else{
				bean.setCommons(configCruxService.getValueByTypeAndKey(ConfigCruxBean.CONFIG_CRUX_TYPE_POP_NEWS,ConfigCruxBean.CONFIG_CRUX_DELETE_ERROR)) ;
			}
	...
	public ModelAndView editDictionary(HttpServletRequest request,HttpServletResponse response) throws IOException{
		...
		try {
			...
			if(StringUtil.isNull(bean.getCommons())){
				bean.setCommons(configCruxService.getValueByTypeAndKey(ConfigCruxBean.CONFIG_CRUX_TYPE_POP_NEWS,ConfigCruxBean.CONFIG_CRUX_KEY_OPERATION_SUCCESS)) ;
			}
			writeJson(response, bean);
		} catch (Exception e) {
