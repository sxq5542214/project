/**
 * 
 */
package com.yd.business.price.service.impl;

import java.math.BigDecimal;
import java.util.Date;
import java.util.List;

import javax.annotation.Resource;

import org.springframework.stereotype.Service;

import com.yd.basic.framework.bean.IOTWebDataBean;
import com.yd.basic.framework.service.BaseService;
import com.yd.business.price.bean.PriceBean;
import com.yd.business.price.service.IPriceService;
import com.yd.iotbusiness.mapper.dao.LmPriceModelMapper;
import com.yd.iotbusiness.mapper.dao.LmPricedetailModelMapper;
import com.yd.iotbusiness.mapper.model.LmPriceModel;
import com.yd.iotbusiness.mapper.model.LmPriceModelExample;
import com.yd.iotbusiness.mapper.model.LmPricedetailModel;
import com.yd.iotbusiness.mapper.model.LmPricedetailModelExample;

/**
 * @author ice
 *
 */
@Service("priceService")
public class PriceServiceImpl extends BaseService implements IPriceService {
	@Resource
	private LmPriceModelMapper priceModelMapper;
	@Resource
	private LmPricedetailModelMapper pricedetailModelMapper;
	

	public int createPrice(PriceBean bean) {
		// TODO Auto-generated method stub
		return 0;
	}

	public int updatePrice(PriceBean bean) {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public int disablePrice(long priceId) {
		PriceBean bean = new PriceBean();
		bean.setP_id(priceId);
		bean.setP_enabled(PriceBean.ENABLED_FALSE);
		return updatePrice(bean);
	}
	@Override
	public int enablePrice(long priceId) {
		PriceBean bean = new PriceBean();
		bean.setP_id(priceId);
		bean.setP_enabled(PriceBean.ENABLED_TRUE);
		return updatePrice(bean);
	}

	@Override
	public IOTWebDataBean queryPriceListByCompany(Integer systemid) {
		IOTWebDataBean result = new IOTWebDataBean();
		
		LmPriceModelExample ex = new LmPriceModelExample();
		LmPriceModelExample.Criteria cri = ex.createCriteria();
		cri.andSystemidEqualTo(systemid);
		List<LmPriceModel> list = priceModelMapper.selectByExample(ex );
		
		result.setData(list);
		
		return result;
	}

	/**
	 * 添加或修改价格
	 */
	@Override
	public int addOrUpdatePrice(PriceBean bean) {
		
		int i = 0 ;

// 简化界面，写死部分数据
bean.setP_base2(bean.getP_base1());	
bean.setP_base3(bean.getP_base1());	
bean.setP_ton1(new BigDecimal(10));
bean.setP_ton2(bean.getP_ton1().multiply(new BigDecimal(2)));
// 写死结束

		bean.setP_price1(bean.getP_base1().add(bean.getP_other1()).add(bean.getP_other2()));
		bean.setP_price2(bean.getP_base2().add(bean.getP_other1()).add(bean.getP_other2()));
		bean.setP_price3(bean.getP_base3().add(bean.getP_other1()).add(bean.getP_other2()));
		
		if(bean.getSeq() == null) {
			bean.setSeq(9);
		}
		bean.setP_updatedate(new Date());
		if(bean.getP_id() == null) {
			bean.setP_createdate(new Date());
			
//			i = priceDao.insertPrice(bean);
//		}else {
//			i = priceDao.updatePrice(bean);
		}
		return i;
	}

	@Override
	public List<PriceBean> queryALLPriceList(PriceBean bean) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public LmPricedetailModel findPriceDetailById(int id) {
		LmPricedetailModelExample model = new LmPricedetailModelExample();
		LmPricedetailModelExample.Criteria cri = model.createCriteria();
		
		cri.andPriceidEqualTo(id);
		List<LmPricedetailModel> list = pricedetailModelMapper.selectByExample(model );
		
		if(list.size()> 0 ) {
			return list.get(0);
		}
		
		return null;
	}

	@Override
	public PriceBean findPriceById(long id) {
		// TODO Auto-generated method stub
		return null;
	}

	
	
	
	
}
