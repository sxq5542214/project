...
public class LoginServiceImpl extends BaseService implements ILoginService{
	...
	private Color getRandColor(int fc, int bc) {
		...
		public OperatorBean login(String username,String password) {
			OperatorExtBean bean = operatorService.findOperatorByNameAndPass(username, password);
			if(bean != null) {
				// 登录用户写入session缓存
		...
		@Override
		public LmOperatorModel loginIOTWeb(String username,String password) {
			LmOperatorModel bean = operatorService.findLmOperatorByNameAndPass(username, password);
			if(bean != null) {
				// 登录用户写入session缓存
				HttpSession session = WebContext.getHttpSession();
				WebContext.setObejctToSession(WebContext.SESSION_ATTRIBUTE_CURRENT_OPERATOR, bean);
				WebContext.setHttpSessionById(session);
				bean.setPass(session.getId());
			}
