...
public class LoginController extends BaseController {
	 ...
	 * IOT界面登录
	 */
	@RequestMapping("**/login/loginByIOTWeb.do")
	public void loginByIOTWeb(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {

		IOTWebDataBean result = new IOTWebDataBean();
		String username = request.getParameter("username");
		String password = request.getParameter("password");
		LmOperatorModel bean = loginService.loginIOTWeb(username, password);
		if(bean == null) {
//			bean = new LmOperator();
			result.setCode(-1);
			result.setMessage("登录失败，请检查用户名或密码");
			
		}else {
			result.setCode(IOTWebDataBean.CODE_IOTWEB_SUCCESS);
			result.setData(bean.getPass());  //pass已被替换为 sessionid
			result.setToken(bean.getPass());
		}
