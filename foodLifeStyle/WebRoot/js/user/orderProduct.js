var second_discount = 0.2;
var low_discount = 0.8;
...
////		alert("亲,您当前积分账户"+user_points+"元,无法足额抵扣该商品价格的20%("+(price/100*second_discount).toFixed(2)+"元),抱歉暂时无法用最低价"+(price/100*low_discount).toFixed(2)+"元购买.\n分享此页面至朋友圈，可立即到账2元积分(可多次重复分享到账).");	
//		if( (pricenow - pricelow) >= 2){
//			alert("亲,分享此页面至朋友圈，支付金额立减2元!最低减至8折！");
//		}else{
...
function addProductInfo(csp){
	...
	for(var i = 0; i < csp.listSupplierBean.length ;i++){
		...
		html +='<div class="Block" style="height: auto;" id="'+sb.id+'" title="'+ sb.product_price +'" discount="'+sb.discount+'" limit="'+sb.product_limit_points+'" givePoints="'+ sb.give_points+'"  offset="'+sb.product_offset_points+'" ';
				 html +='onclick="choseProduct('+sb.id+')" ><div  style="float:right;font-size:10px;color:#919191" >月底清零</div><div  style="float:left;font-size:10px;color:#000000" >'+sb.product_type_name+'</div></br>'+ sb.product_name +'</br><div style="border-top:1px dashed #cccccc;height: 1px;overflow:hidden;margin-top:auto;"></div> ';
				 html +='<div style="font-size:10px;text-decoration:line-through;color:#919191;float:right;">原价:'+sb.product_price/100+'元</div></div>';

 //       html +='onclick="choseProduct('+sb.id+')" ><div  style="float:right;font-size:10px;color:#919191" >月底清零</div><div  style="float:left;font-size:10px;color:#000000" >'+sb.product_type_name+'</div></br>'+ sb.product_name +'</br><span style="font-size:10px;text-decoration:line-through;color:#919191;float:right;">原价:'+sb.product_price/100+'元</span></br><div style="border-top:1px dashed #cccccc;height: 1px;overflow:hidden;margin-top:auto;"></div> ';
