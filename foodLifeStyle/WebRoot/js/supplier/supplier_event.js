...
Art = function(){
	return {
		preview:function(){
			...
			$.post("supplierEvent/find.do",{"id":id},function(data){
				var article = eval('('+data+')');
				$('#title').append(article.title);
				$('#content').append(article.content);
			});
		},
		calcLottery:function(){
			//计算中奖号
			var lotteryNumber = $('#lotteryNumber').val();
			var joinCount =  $('#joinCount').val();
			var id = $('#id').val();
			if(lotteryNumber == null || lotteryNumber == '' || joinCount == null || joinCount == '' || joinCount ==0){
				alert('请正确的填写开奖号和参与人数！');
				return;
			}
			var num = parseInt(lotteryNumber) % parseInt(joinCount);
			
			if(id == null || id == ''){
				alert('开奖号：'+lotteryNumber+'，参与人数：'+joinCount+'，得出的号码为：'+num);
				return;
			}
			
			
			$.post('supplierEvent/queryCode.do',{"eventId":id,"code":num },function(data){
				if(data == null || data == ''){
					alert('无人中此号码:'+num);
				}else{
					data = eval('('+data+')');
					alert('微信名称为：'+data.fromUserName+' 中奖！中奖号：'+num);
				}
		...
		init:function(page){
			Art.initList();
			$.post('supplierEvent/list.do',{"nowpage":page,"title":$('#title').val(),"status":$('#status').val()},function(data){
				var page = eval('('+data+')');
		...
		del:function(id){
			...
			if(flag){
				$.post('supplierEvent/delete.do',{"id":id},function(data){
					if(data=='0'){
						openMsg('提示信息','删除失败');
					}else{
	//					Art.init(1);
...
//			}
		...
		updateWin:function(id){
			parent.document.getElementById("page-frame").src="supplierEvent/toUpdatePage.do?id="+id;
		},
		...
		fileUpload:function(act){
			...
			$.ajaxFileUpload({
				url : 'supplierEvent/fileUpload.do'+params,
				secureuri : false,
