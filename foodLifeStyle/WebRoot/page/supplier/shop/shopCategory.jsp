  ...
  var left = new Vue({
    ...
    data: {
      ...
      <%for (int i = 0; i < productCategoryList.size(); i++) {
        ...
        { name : '<%=bean.getName()%>' } <%if (i != productCategoryList.size() - 1) {%> ,
        <%}
			}%>
  ...
  var right = new Vue({
    ...
    data: {
      ...
      <%String typeName = "";
        ...
        { class : '<%=product.getProduct_category_name()%>',list : [ {id:'<%=product.getId()%>', name:'<%=product.getProduct_name()%>', title : '<%=product.getProduct_title()%>' ,price:'<%=product.getProduct_price()/100d%>', real_price:'<%=product.getProduct_real_price()/100d%>', imgUrl:'<%=product.getProduct_img()%>', productUrl:'product/supplierProduct/toSupplierProductShopInfo.do?id=<%=product.getId()%>' } 
        
        <%} else {%>
        		,{ id:'<%=product.getId()%>', name:'<%=product.getProduct_name()%>', title : '<%=product.getProduct_title()%>' ,price:'<%=product.getProduct_price()/100d%>' , real_price:'<%=product.getProduct_real_price()/100d %>', imgUrl:'<%=product.getProduct_img()%>', productUrl:'product/supplierProduct/toSupplierProductShopInfo.do?id=<%=product.getId()%>' }
        <%}
