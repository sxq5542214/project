...
//		$('.num1').html(2-t);
		...
		var lucky_money = document.getElementById("luckymoney").value;
//		if(r<t*33){
		if(lucky_money == null || lucky_money == ''){
			document.getElementById("prompt").innerHTML="很遗憾！本次充值未达免单活动条件！";
		}else if(lucky_money == 0){
			document.getElementById("prompt").innerHTML="很遗憾！没有中奖！";
		}else{
			document.getElementById("prompt").innerHTML="恭喜您获得了"+document.getElementById("luckymoney").value/100+"元微信红包！<br>分享到朋友圈后即可领取！";
		}
...
//	window.onload = function(){	
	function drawCanvas(){
		...
		//由于无法获取c1的clientWidth，所以变相获取数值
		var topW = $("#top").width();
		var cWidth = $("#c1").width();
		var c1ClientWidth = document.documentElement.clientWidth*topW*cWidth/10000;
		c1.width=c1ClientWidth;
		c1.height=$("#c1").height();
	...
	function initCanvas(){
		...
		ctx.font = "Bold 30px Arial";
				...
				ctx.fillText("刮奖赢免单",c1.width/2,50);
