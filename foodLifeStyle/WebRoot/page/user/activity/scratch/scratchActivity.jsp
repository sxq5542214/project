...
String openid = user == null ? "" : relation.getOpenid();
%>
<!DOCTYPE html>
<html>
	<head>
	...
	<body>
		<img src="page/user/activity/scratch/img/banner1.png" class="banner1"/>
		...
		<img src="page/user/activity/scratch/img/guize.png" class="guize" />
		...
		<script>
			//控制刮卡次数
			var t = 1; 
			var joined = '<%=relation.getStatus() == ActivityUserRelationBean.STATUS_USED%>';
			//初始化所有数据并且随机产生奖品
...
//				if(r<t*33){
					...
					if(joined == 'true'){
						document.getElementById("prompt").innerHTML="您已经参加过活动啦！<br>获得了<%=relation.getProduct_name()%>!";
					}else{
						document.getElementById("prompt").innerHTML="(ಥ_ಥ)来晚了!活动名额已经用尽！下一期免费福利月底不见不散！";
						<%-- document.getElementById("prompt").innerHTML="恭喜您获得了<%=relation.getProduct_name()%>!";
						var ok =document.getElementById("ok");
						ok.style.display = "block";
						//点击领取奖品
						ok.onclick = function () {
							window.location.href="activity/scratchAddPhoneActivity.do?openid=<%=relation.getOpenid()%>";
						}; --%>
					}
//				}else{
...
//				}
	...
	weixinInit.setShareLink('<%=basePath%>wechat/user/liuliang1GActivity.htm?fromOpenid=<%=openid %>');
