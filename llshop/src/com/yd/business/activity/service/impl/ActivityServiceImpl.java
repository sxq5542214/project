...
//		AutoInvokeGetSetMethod.autoInvoke(activity, win);
	 ...
	 */
	@Override
	public ActivityUserRelationBean firstShakeAddPhone(String openid,int relation_id,String phone){
		ActivityUserRelationBean bean = new ActivityUserRelationBean();
		bean.setOpenid(openid);
		bean.setId(relation_id);
		bean = activityDao.findRelationByBean(bean);
		
		if(bean.getStatus() == ActivityUserRelationBean.STATUS_USED){
			return bean;
		}
		
		//判断该该号码是否参与订购过产品
			//获取该号码的其他订购产品,原则上一个号码一个活动只能参与一次  add by bobo
		boolean isJoind = checkPhoneHasJoinedRelation(openid,bean.getActivity_config_id(), phone);
		if(isJoind){
			return bean;	
		}
		...
		if(ad != null){
			...
			//判断商品是否有主备
			ActivityProductBean ap = activityDao.findActivityProductById(bean.getAct_prod_id());
			
			SupplierProductBean sp = supplierProductService.findSupplierProductById(ap.getSupplier_product_id());
			ProductBean product = productService.findProductById(sp.getProduct_id());
			String brandName = productTypeService.findProductTypeById(product.getProduct_brand()).getName();
			
			if(ad.getBrand().equals(brandName)){
				bean.setProduct_id(sp.getId());
			}else if(ap.getOther_spid() != null){
				sp = supplierProductService.findSupplierProductById(ap.getOther_spid());
				product = productService.findProductById(sp.getProduct_id());
				brandName = productTypeService.findProductTypeById(product.getProduct_brand()).getName();
			...
			if(bean.getProduct_id() == null){
				bean.setStatus(ActivityUserRelationBean.STATUS_FAILD);
				bean.setRemark("手机号码与充值的商品不匹配！");
				return bean;
			}
			
			//去订购
			String shakeActivity = "shakeActivity";
			//缓存中没有的，才去订购
			ConcurrentHashMap<String, Object> runningMap = WebContext.getRunningCacheMap();
			OrderProductLogBean orderLog = new OrderProductLogBean();
			if(runningMap.get(shakeActivity+relation_id) == null){
				
				runningMap.put(shakeActivity+relation_id, "running");
				
				if(bean.getStatus() != ActivityUserRelationBean.STATUS_USED){
//					bean.setShare_status(share_type);
					
					UserWechatBean user = userWechatService.findUserWechatByOpenId(openid);
					//创建订购信息记录
					String out_trade_code = userConsumeInfoService.createOutTradeNo(IUserConsumeInfoService.OUTTRADE_TYPE_SHAKE_GIFT, user.getId());
					UserConsumeInfoBean consumeInfo = userConsumeInfoService.createConsumeInfo(bean.getPhone(), 0, bean.getProduct_id(), user.getId(), null, out_trade_code, IUserConsumeInfoService.OUTTRADE_TYPE_SHAKE_GIFT,UserConsumeInfoBean.EVENT_TYPE_USER_ACTIVITY_SHAKE);
					orderLog = orderProductLogService.createOrderProductLogByUserConsumeInfo(consumeInfo, 0, 0, null);
					
					//修改两个定单状态为已支付和待订购
					userConsumeInfoService.updateUserConsumeInfoStatus(UserConsumeInfoBean.STATUS_SUCCESS, out_trade_code);
					orderProductLogService.updateOrderProductLogStatusToPaySuccess(out_trade_code);

					bean.setOrder_code(out_trade_code);
					
					boolean flag = configAttributeService.getBooleanValueByCode(AttributeConstant.CODE_ACTIVITY_RUNNING);
					if(flag)
					{
						//具体的订购
						orderLog = orderService.orderProductByActivity(bean);
					}
					bean.setStatus(orderLog.getStatus());
					bean.setRemark(orderLog.getRemark());
					
					activityDao.updateUserRelateion(bean);
				}
				runningMap.remove(shakeActivity+relation_id);
			}else{
	 ...
	 */
	@Override
	public ActivityUserRelationBean firstShakeShareSuccessToOrderProduct(String openid,int share_type,int relation_id){
//		OrderProductLogBean orderLog = null;
	 ...
	 */
	@Override
	public ActivityUserRelationBean joinFirstShakeActivity(UserWechatBean user){
		
		ActivityConfigBean activityConf = activityConfigService.findActivityConfigByCode(ActivityConfigBean.CODE_FIRSTSHAKE_ACTIVITY);
		ActivityUserRelationBean reBean = new ActivityUserRelationBean();
		reBean.setOpenid(user.getOpenid());
		reBean.setActivity_config_id(activityConf.getId());
