...
//  var givePoints = $("#"+pid).attr("givePoints");
	...
	var coupon_offsetmoney =  $("#coupon_offsetmoney").val();
	...
	if(coupon_offsetmoney == 0 || coupon_offsetmoney == "" || coupon_offsetmoney == null){
		$("#discount_price").html( (price/100 * discount / 100 * coupon_discount / 10 - sub_points).toFixed(2));
	}else{
		if((price/100  - sub_points - coupon_offsetmoney).toFixed(2) <= 0){
			$("#discount_price").html(0.00);
		}else{
			$("#discount_price").html( (price/100  - sub_points - coupon_offsetmoney).toFixed(2));
		}
...
function checkPhone(){
	...
	if(phone.length == 11){
		...
		$("#product_id").val('');
		$.ajax({
			url : 'supplier/queryProductByPhone.do',
			data : {
...
function addProductInfo(csp){
	...
	for(var i = 0; i < csp.listSupplierBean.length ;i++){
	    ...
	    var coupon_discount_product = sb.coupon_discount_product;
	    if(coupon_discount_product ==1){
	    	...
	    	html +='<div class="Block" style="height: auto;width:45%;color:#919191;" id="'+sb.id+'" title="'+ sb.product_price +'" realprice="'+sb.product_real_price +'" discount="'+sb.discount+'" limit="'+sb.product_limit_points+'" givePoints="'+ sb.give_points+'" offset="'+sb.product_offset_points+'"  ';
	        html +=' ><div  style="float:right;font-size:10px;color:#919191;" >月底清零</div><div  style="float:left;font-size:10px;	color:#919191;" >'+sb.product_type_name+'</div><br><div style="font-size: 14px;">'+ sb.product_name +'</div> ';
	        html +='<div style="float:center;color:#919191;">该优惠卷不可用</div></div>';
	    }
...
function payInfo(){
	...
	location.href = "supplier/toOrderConfirm.do?phone="+phone+"&spid="+spid+"&openid="+openid+"&coupon_id="+coupon_id+"&coupon_record_id="+coupon_record_id+"&customer_id="+customer_id;
