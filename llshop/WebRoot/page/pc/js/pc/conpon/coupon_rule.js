...
function commitData(item){
	coupon_id=$("select[name=couponrule_coupon_id]").val();
	rule_name=$("input[name=couponrule_rule_name]").val();
	explain=$("input[name=couponrule_explain]").val();
	status=$("select[name=couponrule_status]").val();
	type=$("input[name=couponrule_type]").val();
	begin_time=$("input[name=couponrule_begin_time]").val();
	end_time=$("input[name=couponrule_end_time]").val();
	SQL=$("input[name=couponrule_SQL]").val();
	...
	$(".couponrule").focus(function(){
		console.log($(this).closest(".form-group"));
		if($(this).closest(".form-group").hasClass("has-error")){
			$(this).closest(".form-group").removeClass("has-error");
		}
	});
	var id = $("input[name=couponrule_id]").val();
	$.ajax({
		type : "POST",
		url : "../../admin/supplier/supplierCouponController/editCouponRule.do",
		data : {
			"id" : id,
			"coupon_id" : coupon_id,
			"rule_name":rule_name,
...
function addItemToList(result){
	...
	$(body_list).children("td").each(function(n,e){
		...
		var couponrule_name = class_name.substr(17,class_name.length);
		if($(e).children().length > 0){
			$(e).children("input").val(result[couponrule_name]);
			$("select[name="+couponrule_name+"]").val($(e).children("input").val());
		}else{
			$(e).text(result[couponrule_name]);
		}
	});
...
function editCouponRule(item){
	...
	$(item).closest(".gradeA").find("td").each(function(n,e){
		...
		var item_name = "couponrule_"+class_name;
		if($(e).children().length > 0){
			$("input[name="+item_name+"]").val($(e).children("input").val());
			$("select[name="+item_name+"]").val($(e).children("select").val());
		}else{
...
function deleteitem(item){
	if(confirm("确定删除数据？")){
		...
		$.ajax({
			type : "POST",
			url : "../../admin/supplier/supplierCouponController/deleteCouponRule.do",
			data : {
				"id" : id
			},
			success : function(data) {
